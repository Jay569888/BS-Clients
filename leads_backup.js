...
window._renderIntakeTable=function(leads){
  ...
  const chkHdrs=['Call','VM','Email','Text','Upl'];
  const chkWidths=['36px','36px','42px','36px','42px'];
  const rows=leads.map(l=>{try{
    ...
    const levelSelectHtml=`<select style="font-size:10px;padding:2px 4px;border-radius:3px;border:1px solid var(--border);background:var(--card);max-width:110px" onchange="window.updateIntakeLeadField('${l.id}','level',this.value)"><option value="" ${!l.level?'selected':''}>— Select —</option>${(window.state.templates||[]).map(t=>`<option value="${t.name}" ${l.level===t.name?'selected':''}>${t.name}</option>`).join('')}</select>`;
