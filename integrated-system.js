...
class ErrorHandler {
  ...
  isCritical(error) {
    return /authentication|session expired|network error|failed to fetch/i.test(error.message);
  }
...
window.addEventListener('error', (event) => {
  window.errorHandler.handle(event.error, { type: 'unhandled', source: event.filename });
});

window.addEventListener('unhandledrejection', (event) => {
  window.errorHandler.handle(new Error(event.reason), { type: 'unhandled-promise' });
});
