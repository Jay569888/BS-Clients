...
window.enterApp = async function(userKey) {
  ...
  try {
    ...
    // ── Clear checkboxes + EOD on every login ───────────────────────────────
    // Resets call/vm/emailChk/text/upload on all leads across all tabs,
    // and wipes the EOD log — then pushes both to the backend.
    (async () => {
      try {
        ...
        allTabs.forEach(tab => {
          (window.state.leads[tab] || []).forEach(lead => {
            ...
            if (needsReset) {
              ...
              updates.push({ action:'updateLead', tab, lead: JSON.stringify(lead) });
            }
          });
        });
        // Clear EOD
        window.state.eod = [];
        // Also clear the local highlight state from previous session
        try { localStorage.removeItem('bs_hl_clicked'); } catch(e) {}
        try { localStorage.removeItem('bs_hl_search'); } catch(e) {}
        window._clickedLeadId = null;
        window._searchHighlightIds = new Set();
        // Push to backend
        const saves = [...updates, { action:'saveMeta', eod:'[]' }];
        for (const params of saves) {
          window._queueSave(params);
        }
      } catch(e) { console.warn('Login reset failed:', e.message); }
    })();
