...
window.sendGmailDirect = async function(to, subject, body) {
  ...
  // Normalize body to fix double-spacing from contenteditable + template combo (idempotent).
  ...
  // This means templates no longer need to include the sig — Gmail's saved sig is used.
  try {
    if (window._fetchGmailSignature) {
      const sig = await window._fetchGmailSignature();
      if (sig && sig.trim()) {
        // Skip if the body already contains the signature or a clear sig delimiter
        const bodyText = body.replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').toLowerCase();
        const sigText  = sig.replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').toLowerCase();
        const sigFingerprint = sigText.slice(0, 60); // first 60 chars of sig text
        if (sigFingerprint && !bodyText.includes(sigFingerprint)) {
          // Body now ends without a trailing margin/spacer.
          // Use a single empty <p> spacer to create one clean blank line before the signature.
          // (Matches the spacer pattern used throughout the normalized body — guaranteed
          // to render correctly in Gmail even if inline margins get stripped.)
          body = body + '<p style="margin:0;padding:0;line-height:1.5">&nbsp;</p>' + sig;
        }
      }
    }
  } catch(e) { /* signature optional — proceed without */ }
...
// ── Inbox state ───────────────────────────────────────────
window._inboxMessages  = []; // { msgId, threadId, from, fromEmail, subject, date, bodyHtml, leadId, leadName, leadTab }
window._inboxAnswered  = new Set(); // msgIds already replied to
...
window._checkInboxReplies = async function() {
  ...
  try {
    ...
    for (const msg of messages.slice(0, 20)) {
      // Skip already processed or answered
...
window._extractEmailBody = function(payload) {
  ...
  // Parts — prefer text/html, fall back to text/plain
  if (payload.parts) {
...
window._showInboxToast = function(lead, subject) {
  ...
  toast.innerHTML = `
    <div style="font-weight:700;font-size:12px;margin-bottom:4px">📬 New email from ${window._escAttr(lead.name)}</div>
    <div style="font-size:11px;opacity:0.85">${window._escAttr(subject)}</div>
    <div style="font-size:10px;opacity:0.7;margin-top:4px">Click to read & reply</div>
  `;
...
// ── Render inbox panel ────────────────────────────────────
window._activeInboxMsg = null; // currently selected message
...
window._renderInboxPanel = async function() {
  ...
  if (!unread.length) {
    overlay.innerHTML = `<div style="background:var(--card);border-radius:10px;padding:28px;max-width:480px;width:100%;box-shadow:0 12px 40px var(--shadow-lg);text-align:center">
      <div style="font-size:32px;margin-bottom:12px">✅</div>
      <div style="font-size:15px;font-weight:600;color:var(--primary);margin-bottom:6px">All caught up!</div>
      <div style="font-size:12px;color:var(--text-muted);margin-bottom:18px">No unread emails from leads.</div>
      <button class="btn btn-outline btn-sm" onclick="window._closeInboxPanel()">Close</button>
    </div>`;
    window._updateInboxBadge();
    return;
  }

  // Set default active message
    ...
    <div style="display:flex;flex:1;min-height:0;overflow:hidden">
      ...
      <div style="width:240px;flex-shrink:0;border-right:1px solid var(--border);overflow-y:auto;background:var(--primary-light)">
        ${unread.map(m => {
          ...
          return `<div onclick="window._selectInboxMsg('${m.msgId}')"
      ...
      <div style="flex:1;display:flex;flex-direction:column;min-width:0;overflow:hidden">
        ...
        <div style="border-top:2px solid var(--border);padding:12px 16px;flex-shrink:0;background:var(--primary-light)">
          ...
          <div style="display:flex;gap:6px;margin-bottom:8px;align-items:center">
            ...
            <button onclick="window._applyInboxTemplate('${active.leadId}','${active.msgId}')"
              style="background:var(--primary);color:#fff;border:none;border-radius:5px;padding:4px 10px;cursor:pointer;font-size:11px;font-weight:600">Apply</button>
          </div>
          ...
          <!-- Reply body -->
          <div id="inboxReplyBody"
            contenteditable="true"
            style="min-height:90px;max-height:180px;overflow-y:auto;border:1px solid var(--border);border-radius:6px;padding:10px;font-size:12px;background:var(--card);outline:none;line-height:1.6"
            placeholder="Type your reply here...">${sigHtml}</div>

          <!-- Actions -->
          <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:8px">
            <button onclick="window._markInboxRead('${active.msgId}','${active.leadId}')"
              style="background:var(--success-bg);color:var(--success-text);border:1px solid var(--success);border-radius:6px;padding:5px 12px;cursor:pointer;font-size:11px;font-weight:600">
              ✓ Mark Read (no reply)
            </button>
            <button id="inboxSendBtn"
              onclick="window._sendInboxReply('${active.msgId}','${active.leadId}','${active.leadEmail.replace(/'/g,"\\'")}','${active.subject.replace(/'/g,"\\'")}','${active.threadId}')"
              style="background:var(--primary);color:#fff;border:none;border-radius:6px;padding:5px 16px;cursor:pointer;font-size:11px;font-weight:700">
              ✉️ Send Reply
            </button>
          </div>
        </div>
...
window._sendInboxReply = async function(msgId, leadId, toEmail, originalSubject, threadId) {
  ...
  // 2. Mark emailChk + update ffup
  if (lead) {
    lead.emailChk  = true;
    lead._prevFFUP = lead.ffup || '';
    lead.ffup      = window.todayMD();
    // Also mark intakeChase if intake
    if (tab === 'intake') {
  ...
  // 4. Mark original as read in Gmail
  await window._markGmailRead(msgId);

  // 5. Remove from inbox panel (answered)
  ...
  // 7. Re-render panel (shows next unread or "all clear")
  window._renderInboxPanel();
};
