...
window.sendGmailDirect = async function(to, subject, body) {
    ...
    if (window._fetchGmailSignature) {
      ...
      if (sig && sig.trim()) {
        ...
        if (sigFingerprint && !bodyText.includes(sigFingerprint)) {
          // Strip the trailing bottom margin off the last <p> / <ul> / <ol> / <li>
          // in the body so the signature sits one normal blank line below — not a chasm.
          let tight = body
            .replace(/(<p\b[^>]*?)style="([^"]*?)margin:0 0 12px 0;([^"]*?)"([^>]*>(?:(?!<\/p>)[\s\S])*<\/p>)\s*$/i,
                     '$1style="$2margin:0;$3"$4')
            .replace(/(<ul\b[^>]*?)style="([^"]*?)margin:0 0 12px 24px;([^"]*?)"([^>]*>(?:(?!<\/ul>)[\s\S])*<\/ul>)\s*$/i,
                     '$1style="$2margin:0 0 0 24px;$3"$4')
            .replace(/(<ol\b[^>]*?)style="([^"]*?)margin:0 0 12px 24px;([^"]*?)"([^>]*>(?:(?!<\/ol>)[\s\S])*<\/ol>)\s*$/i,
                     '$1style="$2margin:0 0 0 24px;$3"$4');
          body = tight + '<br>' + sig;
        }
      }
  ...
  // Wrap the whole email in a single font container so Gmail uses a consistent font
  // and size — matching what you see in the composer/preview. Doesn't restructure
  // your content at all; just sets the rendering defaults.
