...
window.renderEOD = function() {
  ...
  const log = Array.isArray(window.state.eod) ? window.state.eod : [];
  if (!log.length) {
  ...
  el.innerHTML = log.map(e => `<div class="eod-entry">
    ...
    <div class="eod-time">${e.time||''} · <span class="badge badge-${e.tab||'pc'}">${window.TAB_LABELS[e.tab]||e.tab||''}</span></div>
  </div>`).join('');
};
