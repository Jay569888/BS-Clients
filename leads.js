...
window._renderIntakeTable=function(leads){
  ...
  const rows=leads.map(l=>{try{
    ...
    if(ev._missingOverride){misDisplay=ev._missingOverride;misColor='var(--text)';misWeight='400';}
    const gdriveDisplay=l.gdrive?`<a href="${l.gdrive}" target="_blank" style="color:var(--primary);text-decoration:none;font-weight:500">🔗 Open</a>`:`<span style="color:var(--text-muted);cursor:pointer" onclick="window.editGdriveLink('${l.id}','intake')">+ Add Link</span>`;
    ...
    const levelSelectHtml=`<select style="font-size:10px;padding:2px 4px;border-radius:3px;border:1px solid var(--border);background:var(--card);max-width:110px" onchange="window._handleLevelChange('intake','${l.id}',this)"><option value="" ${!l.level?'selected':''}>— Select —</option>${(window.state.templates||[]).map(t=>`<option value="${t.name}" ${l.level===t.name?'selected':''}>${t.name}</option>`).join('')}${(l.level && !(window.state.templates||[]).some(t=>t.name===l.level))?`<option value="${l.level}" selected>${l.level}</option>`:''}<option value="__CUSTOM__" style="font-style:italic;color:var(--primary);font-weight:600">✏️ Custom…</option></select>`;
  ...
  return`<table><thead><tr>
    ...
    <th>Evidence Link</th><th>G-Drive</th>
...
window.editGdriveLink=function(id,tab){const lead=window.findLeadById(id);if(!lead)return;const newUrl=prompt(`Evidence Link for ${lead.name}:`,lead.gdrive||'');if(newUrl===null)return;lead.gdrive=newUrl.trim();window._patchRowDOM(tab,lead);try{window.apiWithRetry({action:'updateLead',tab,lead:JSON.stringify(lead)});}catch(e){}if(newUrl)window.showSuccess('Evidence Link Saved',`Updated for ${lead.name}.`);};
...
window.renderLeadTable=function(tab,leads){
  ...
  const rows=leads.map(l=>{try{
    ...
    if(ev._missingOverride){misDisplay=ev._missingOverride;misColor='var(--text)';misWeight='400';}
    const gdriveDisplay=l.gdrive?`<a href="${l.gdrive}" target="_blank" style="color:var(--primary);text-decoration:none;font-weight:500">🔗 Open</a>`:`<span style="color:var(--text-muted);cursor:pointer" onclick="window.editGdriveLink('${l.id}','${tab}')">+ Add Link</span>`;
...
window._editEveLink=function(id,tab){const lead=window.findLeadById(id);if(!lead)return;const newUrl=prompt(`G-Drive link for ${lead.name}:`,lead.eve||'');if(newUrl===null)return;lead.eve=newUrl.trim();window._patchRowDOM(tab,lead);try{window.apiWithRetry({action:'updateLead',tab,lead:JSON.stringify(lead)});}catch(e){}if(newUrl)window.showSuccess('G-Drive Link Saved',`Updated for ${lead.name}.`);};
...
window._applyNDayNoticeAfterSend = function(lead, tab, templateName, subject) {
  ...
  // Is the email being sent a "For Review" email? Only review emails should
  // keep / set the green row. Any other email means the lead is no longer
  // in the "for review" state, so we clear the green highlight.
  const isReviewEmail = /review/i.test(templateName || '') || /review/i.test(subject || '');
  ...
  // CLEAR GREEN REVIEW STATE — a different (non-review) email went out, so
  // the lead is no longer "For Review". Remove the green row + review note.
  if (!isReviewEmail && (lead._reviewInitiated || lead.rowAlert === 'review')) {
    lead.rowAlert = '';
    lead._reviewInitiated = false;
