...
window._renderIntakeTable=function(leads){
  ...
  const rows=leads.map(l=>{try{
    ...
    return`<tr id="lead-row-${l.id}" tabindex="0" data-id="${encodeURIComponent(l.id)}" data-tab="intake" style="${rowBg?'background:'+rowBg:''}">
      ...
      <td ondblclick="window.openEditLeadModal('intake','${l.id}');navigator.clipboard&&l.name&&navigator.clipboard.writeText(l.name)" style="font-weight:600;color:var(--primary);cursor:pointer;${l.nameHighlight?'background:'+l.nameHighlight+';':cellBg}">${starBadge}${l.prioTomorrow?'❤️ ':''}${replyBadge}${l.name||''}</td>
...
window.renderLeadTable=function(tab,leads){
  ...
  const rows=leads.map(l=>{try{
    ...
    return`<tr id="lead-row-${l.id}" tabindex="0" data-id="${l.id}" data-tab="${tab}" class="${rowClass}" style="background:${rowBg}">
      ...
      <td class="editable" ondblclick="window.inlineEdit(this,'${tab}','${l.id}','name');navigator.clipboard&&l.name&&navigator.clipboard.writeText(l.name)" style="font-weight:600;color:var(--primary);${l.nameHighlight?'background:'+l.nameHighlight+';':cellBg}">${starBadge}${l.prioTomorrow?'❤️ ':''}${replyBadge}${l.name||''}</td>
      <td style="${cellBg}">${phoneHtml}</td>
      <td style="font-size:10px;${cellBg}">${l.email?`<span class="copy-cell" data-copy="${l.email.replace(/"/g,'&quot;')}" data-copy-kind="email" title="Double-click to copy">${l.email}</span>`:''}</td>
