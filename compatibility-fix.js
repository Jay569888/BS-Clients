        ...
        try {
          ...
          try {
            ...
            if (lead) {
              ...
              // If the scheduled email is NOT a review email, clear any green
              // "For Review" state — the lead has moved on to a different email.
              const isReview = /review/i.test(scheduled.template || '') || /review/i.test(scheduled.subject || '');
              if (!isReview && (lead._reviewInitiated || lead.rowAlert === 'review')) {
                lead.rowAlert = '';
