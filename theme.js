...
window.THEME_PRESETS = {
  navy:  { label:'Navy (Default)', primary:'#1e3a5f', bg:'#f5f5f5', card:'#ffffff', text:'#1a1a1a', border:'#e5e7eb', rowHover:'#f0f7ff', font:'Inter' },
  dark:  { label:'Dark Mode',      primary:'#3b82f6', bg:'#0f172a', card:'#1e293b', text:'#f1f5f9', border:'#334155', rowHover:'#1e3a2f', font:'Inter' },
  rose:  { label:'Rose',           primary:'#be185d', bg:'#fff0f6', card:'#ffffff', text:'#1a1a1a', border:'#fbcfe8', rowHover:'#fce7f3', font:'Inter' },
  green: { label:'Forest Green',   primary:'#166534', bg:'#f0fdf4', card:'#ffffff', text:'#1a1a1a', border:'#bbf7d0', rowHover:'#dcfce7', font:'Inter' },
  slate: { label:'Slate',          primary:'#475569', bg:'#f8fafc', card:'#ffffff', text:'#1a1a1a', border:'#e2e8f0', rowHover:'#f1f5f9', font:'Inter' },
};

window.FONT_OPTIONS = ['Inter','Georgia','Arial','Verdana','Trebuchet MS','Courier New','Times New Roman'];
...
window.renderThemeSettings = function() {
  ...
  const fields = ['primary','bg','card','text','border','rowHover'];
  fields.forEach(f => {
    const el = document.getElementById('tp-' + f);
    if (el) el.value = saved[f] || '#ffffff';
  });
